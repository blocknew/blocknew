...
#if defined(NDEBUG)
# error "Blocknew cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Blocknew Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s BND additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    CAmount nSubsidy = (nHeight <= 2000 ? 8400000 : 10000) * COIN;
...
bool IsInitialBlockDownload()
{
    ...
    if (fImporting || fReindex) {
        LogPrintf("Leaving InitialBlockDownload (latching to true) fImporting= fReindex=\n");
        return true;
    }

    if (chainActive.Tip() == nullptr) {
        LogPrintf("Leaving InitialBlockDownload (latching to true) chainActive.Tip() is nullptr\n");
        return true;
    }

    if (chainActive.Tip()->nChainWork < nMinimumChainWork) {
        LogPrintf("Leaving InitialBlockDownload (latching to true) chainActive.Tip()->nChainWork is smaller than\n");
        return true;
    }

    if (chainActive.Tip()->GetBlockTime() < (GetTime() - nMaxTipAge)) {
        LogPrintf("Leaving InitialBlockDownload (latching to true) chainActive.Tip()->GetBlockTime() is smaller than\n");
        return true;
    }
...
void ThreadScriptCheck() {
    RenameThread("blocknew-scriptch");
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.BIP34Height)
    {
        ...
        if (block.vtx[0]->vin[0].scriptSig.size() < expect.size() ||
            ...
            std::cout << "scriptSig.size=" << block.vtx[0]->vin[0].scriptSig.size() << " scriptSig[0]=" << block.vtx[0]->vin[0].scriptSig[0] << " expect.size=" << expect.size() << " expect[0]=" << expect[0] << std::endl;
            return state.DoS(100, false, REJECT_INVALID, "bad-cb-height", false, strprintf("block height %d mismatch in coinbase", nHeight));
        }
    }
