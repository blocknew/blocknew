...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s blocknew-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  blocknew-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded blocknew transaction") + "\n" +
              "  blocknew-tx [options] -create [commands]   " + _("Create hex-encoded blocknew transaction") + "\n" +
              "\n";
        ...
        for (unsigned int previdx = 0; previdx < prevtxsObj.size(); previdx++) {
...

            {
                ...
                if (prevOut.exists("amount")) {
                    blocknew.out.nValue = AmountFromValue(prevOut["amount"]);
                }
                blocknew.nHeight = 1;
