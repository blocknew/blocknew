    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        // Do a random modification.
        {
            ...
            if (InsecureRandRange(5) == 0 || coin.IsSpent()) {
                Coin blocknew;
                blocknew.out.nValue = InsecureRand32();
                blocknew.nHeight = 1;
                if (InsecureRandRange(16) == 0 && coin.IsSpent()) {
                    blocknew.out.scriptPubKey.assign(1 + InsecureRandBits(6), OP_RETURN);
                    BOOST_CHECK(blocknew.out.scriptPubKey.IsUnspendable());
                    added_an_unspendable_entry = true;
                } else {
                    blocknew.out.scriptPubKey.assign(InsecureRandBits(6), 0); // Random sizes so we can test memory usage accounting
                    (coin.IsSpent() ? added_an_entry : updated_an_entry) = true;
                    coin = blocknew;
                }
                stack.back()->AddCoin(COutPoint(txid, 0), std::move(blocknew), !coin.IsSpent() || InsecureRand32() & 1);
            } else {
